...
struct particle_path {
  ...
  particle_path(const int z0, const int a0, const double e0, const double dt, const double absorption_e,
              const std::vector<double> &change_points,
              const std::vector<int> &interval_materials,
              std::vector<Material> &materials)
      : a(a0), z_c(z0), ix(1), energy(1), s(1), x(1), omega(1), u(3, 0), z(3, 0), w(3, 0) {
  ...
  void reset(const int a0, const int z0, const double e0, const std::vector<double> x0,
  ...
  double spherical_bm(const double dt, gsl_rng *gen,
    ...
    double bethe_bloch_energy_loss =  mat.bethe_bloch(energy[ix - 1]) * time_step;
    energy[ix] =
        energy[ix - 1] -
        fmin(fmax(bethe_bloch_energy_loss +
                 sqrt(time_step) * mat.energy_straggling_sd(energy[ix - 1]) *
                     gsl_ran_gaussian_ziggurat(gen, 1),
             0), 2*bethe_bloch_energy_loss);
    energy[ix] = fmax(energy[ix], 0);
    s[ix] = (energy[ix - 1] - energy[ix])/mat.density;
    ix++;
    return time_step;
  }

  void simulate(const double dt, const double absorption_energy,
               const std::vector<double> &change_points,
               const std::vector<int> &interval_materials,
               const std::vector<Material> &materials, gsl_rng *gen, std::vector<std::vector<std::vector<double>>> &secondary_particles) {
    ...
    while (energy[ix - 1] > absorption_energy) {
      while (fabs(x[ix - 1][0] - change_points[material_index]) < 1e-9) {
        material_index++;
      } while (fabs(x[ix - 1][0] - change_points[material_index - 1]) <
                 1e-9) {
        material_index--;
      }
      time_step = spherical_bm(
          dt, gen, materials[interval_materials[material_index - 1]],
      ...
      if (energy[ix - 1] > absorption_energy) {
        ...
        if (gsl_rng_uniform(gen) < 1 - exp(-alpha * time_step)) {
          if (gsl_rng_uniform(gen) < rutherford_elastic_jump_rate / alpha) {
           ...
           materials[interval_materials[material_index - 1]]
                .nonelastic_scatter(omega[ix - 1], energy[ix - 1], gen, secondary_particles);
          }
        }
      }
    }
