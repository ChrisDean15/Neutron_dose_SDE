...
struct particle_bank {
    particle_bank(const double e0_m_3sd, const double dt0, const double absorption_e0, std::vector<std::vector<Material>> &materials0, std::vector<std::vector<int>> charge_mass_numbers0,const std::vector<double> &change_points_x0,
              const std::vector<double> &change_points_y0,
              const std::vector<std::vector<int>> &interval_materials0)
        : dt(dt0),absorption_e(absorption_e0), change_points_x(change_points_x0), change_points_y(change_points_y0), interval_materials(interval_materials0), particles(materials0.size()), materials(materials0), charge_mass_numbers(charge_mass_numbers0),
          particle_traject(charge_mass_numbers0[0][0], charge_mass_numbers0[0][1], e0_m_3sd, dt0, absorption_e0, change_points_x0, change_points_y0, interval_materials0, materials0[0]) {}
    ...
    void evaluate_particles_proton_neutron_storage(gsl_rng *gen){
        while (!particles[0].empty()) {
            particle_traject.reset(charge_mass_numbers[0][0], charge_mass_numbers[0][1], particles[0].back().energy, 
            ...
            particle_traject.simulate(dt, absorption_e, change_points_x,change_points_y, interval_materials,
    ...
    std::vector<double> change_points_x, change_points_y;
